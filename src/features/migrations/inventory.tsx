...
// Targets from the Bootstrap → TUM UI/Tailwind quick reference in Artemis's client-development guideline.
const bootstrapTargets: [RegExp, string][] = [
  ...
  [/^form-(group|label|text)|^col-form-label/, 'tum-ui-form-field'],
  [/^input-group/, 'tum-ui-input-group'],
  [/^(valid|invalid)-feedback/, 'tum-ui-form-field'],
  [/^(row|col|g[xy]?-)/, 'grid grid-cols-12 + col-span-* / flex + gap-*'],
  ...
  [/^nav/, 'tum-ui-tabs'],
  [/^(pagination|page-)/, 'tum-ui-paginator'],
  [/^spinner-/, 'tum-ui-progress-spinner'],
  [/^popover/, 'tum-ui-popover'],
  [/^toast/, 'tum-ui-message'],
  [/^list-group/, 'tum-ui-list'],
  [/^accordion|^collapse/, 'tum-ui-panel'],
  [/^[hw]-(25|50|75|100)$/, 'w-full / h-full or an explicit size'],
  [/^(close|btn-close)$/, 'tum-ui-button'],
  [/^visually-hidden/, 'sr-only'],
  [/^text-truncate/, 'truncate'],
]
const bootstrapTarget = (name: string) =>
  bootstrapTargets.find(([re]) => re.test(name))?.[1] ?? ''

// PrimeNG and ng-bootstrap names map to a kit selector only when the kit ships that selector.
const kitTarget = (name: string, kit: Set<string>) => {
  const candidates = /^p-/.test(name)
    ...
    : /^ngb-/.test(name)
      ...
      : /^p[A-Z]/.test(name)
        ...
        : /^ngb[A-Z]/.test(name)
          ? [`tumUi${name.slice(3)}`]
          : /^Ngb([A-Z]\w*?)(Module)?$/.test(name)
            ? [
                `tum-ui-${name
                  .replace(/^Ngb|Module$/g, '')
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Every legacy class, element, directive and import still in the client
          at this snapshot, with the target from the Artemis migration guideline
          where one exists.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Tabs defaultValue="bootstrap">
          <TabsList className="flex h-auto flex-wrap">
            ...
            <TabsTrigger value="tumUi">
              TUM UI kit ({detail.inventory.tumUi.length} of {detail.kit.length}{' '}
              used)
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="ngBootstrap" className={scroll}>
            <InventoryTable
              entries={detail.inventory.ngBootstrap}
              target={(n) => kitTarget(n, kit)}
              targetLabel="Kit equivalent"
            />
          </TabsContent>
