...
- **Unit** — one `@Component` or `@Directive` declaration: its `templateUrl`, `styleUrl(s)`
  ...
  per module but are not units. Totals and modules count every file once; a template or
...
- **Module** — the first directory below `src/main/webapp/app` (course, exam, …); `app` for root
  ...
  module's per-commit row, units, pages, next steps and inventory from its units; lock counts and
  pages of a scoped snapshot are derived from its detail.
...
State lives in the hash query (`view`, `snapshot`, `compare`, `module`). Both commit pickers
...
| Modules      | Modules table (sort by legacy-free share, Δ legacy-free, Δ hits, since adoption, last progress or any library) with stage bar, units per library and locked with their change, last progress (latest commit that reduced hits or made a unit legacy-free, marked after four idle weeks); heatmap of Bootstrap hits per kind of work plus SCSS, PrimeNG and ng-bootstrap occurrences (row-normalised). Clicking a module scopes the dashboard to it and this view then shows the module: units by stage with selectors, library usage with kit components, imported units with hits, shared stylesheets and lockable directories |
| Pages        | Routed pages legacy-free / components left / blocked / Bootstrap per module, the global shell's hits, and every page (selector with route, state, hits in the page, imported and in layouts, a popover listing the exact units to fix) sorted by state and remaining work                                                                                                                                                                                                                                                                                                                                                       |
| Next steps   | Quick wins (units with at most three hits that import nothing with Bootstrap, with the exact changes; sort by module to batch a pull request), lockable directories with copyable lock entries and "new since comparison", shared units with Bootstrap that block the most (with units unblocked per hit), kit gaps (legacy usages no kit component covers), modules closest to legacy-free (all modules only)                                                                                                                                                                                                                  |
